// Dynamic PCM WAV Ringtone Sound Asset Generator for Incoming Job Request Alert Tone
// Synthesizes a loud, distinctive multi-phrase phone ringtone (22.05kHz PCM WAV)
...
export function generateIncomingRequestWavBase64() {
  ...
  // 2. Ringtone Synthesis Algorithm
  // Creates a loud, dual-tone phone ring (850Hz + 1250Hz) with urgent trill accents
  for (let i = 0; i < totalSamples; i++) {
    const t = i / sampleRate;
    const cycle = t % 2.0; // 2-second sub-cycle repeated twice
    ...
    if (cycle >= 0.0 && cycle < 0.4) {
      // First ring burst: 850 Hz + 1250 Hz dual-tone ring
      freq1 = 850;
      freq2 = 1250;
      const env = Math.sin((cycle / 0.4) * Math.PI);
      amp = env * 0.95;
    } else if (cycle >= 0.45 && cycle < 0.85) {
      // Second ring burst: 950 Hz + 1350 Hz dual-tone ring
      freq1 = 950;
      freq2 = 1350;
      const env = Math.sin(((cycle - 0.45) / 0.4) * Math.PI);
      amp = env * 1.0;
    } else if (cycle >= 0.95 && cycle < 1.45) {
      // Urgent trill: rapid alternation between 1400 Hz and 1750 Hz
...
export function getRawBase64String() {
  const dataUri = generateIncomingRequestWavBase64();
  return dataUri.replace('data:audio/wav;base64,', '');
}
