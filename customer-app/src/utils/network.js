  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (attempt < retries && isTemporaryFailure(null, response.status)) {
          if (__DEV__) console.log(`[Network Retry] Retrying in ${delay}ms due to server status ${response.status}...`);
